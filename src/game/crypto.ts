...
/** Dependency-free SHA-256 for deterministic runtimes without SubtleCrypto. */
export function sha256HexSync(value: string): string {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      ...
      const earlier = schedule[index - 2] as number
...
/** Uses Web Crypto where available and the exact same SHA-256 fallback elsewhere. */
export async function sha256Hex(value: string, options: HashOptions = {}): Promise<string> {
  const subtle = globalThis.crypto?.subtle
  if (!options.forceFallback && subtle) {
    try {
      ...
      // Sandboxed webviews can expose SubtleCrypto while rejecting operations.
    }
  }
...
export function randomHex(bytes = 32): string {
  if (!Number.isInteger(bytes) || bytes < 16) {
    throw new RangeError('Commitment salts require at least 16 random bytes')
  }
  if (!globalThis.crypto?.getRandomValues) {
    throw new Error('Secure random generation is unavailable; provide a server-generated salt')
  }
